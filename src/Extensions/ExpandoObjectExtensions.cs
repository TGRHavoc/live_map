    ...
    public static class ExpandoObjectExtensions
    {
        public static T FromExpando<T>(this ExpandoObject expando) where T : class, new()
        {
            ...
            var properties = typeof(T)
                ...
                .ToDictionary(pi => pi.GetCustomAttribute<JsonPropertyNameAttribute>()?.Name?.ToLower() ?? pi.Name.ToLower());
            ...
            foreach (var kvp in expando)
            {
                ...
                if (val != null && properties.TryGetValue(name, out PropertyInfo prop))
                {
